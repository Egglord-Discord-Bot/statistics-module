...
module.exports = async (bot) => {
	...
	const srvOverview = await getGuildHistory(bot);
	...
	router.get('/', async (req, res) => {
		...
		const data = {
			BOT: {
				...
				GUILDCOUNTTODAY: bot.guilds.cache.filter(g => g.members.me.joinedTimestamp >= new Date() - 86400000).size,
				USERCOUNT: bot.guilds.cache.reduce((a, g) => a + g.memberCount, 0).toLocaleString(),
				USERCOUNTTODAY: userOverview.filter(u => u.joinedTimestamp >= new Date() - 86400000).length,
			},
			MEMORY: {
